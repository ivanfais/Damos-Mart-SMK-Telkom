...
export class OrdersService {
   ...
   */
  async createOrder(userId: string, data: { cartItemIds: string[]; paymentMethod: 'QRIS' | 'CASH_AT_COUNTER'; notes?: string | null }) {
    ...
    if (cartItems.length === 0) {
      throw new AppError(404, 'CART_ITEMS_NOT_FOUND', 'Selected cart items not found or do not belong to you');
    }
   ...
   */
  async processPayment(userId: string, orderId: string, paymentMethod: 'QRIS' | 'CASH_AT_COUNTER') {
    ...
    if (order.status !== 'PENDING') {
      throw new AppError(400, 'ORDER_ALREADY_PROCESSED', `Cannot pay for order with status: ${order.status}`);
    }
   ...
   */
  async getStudentOrders(userId: string, filters: { status?: string; isPreorder?: boolean; page: number; limit: number }) {
  ...
  async cancelOrder(userId: string, orderId: string) {
    ...
    if (order.status !== 'PENDING') {
      throw new AppError(400, 'ORDER_CANNOT_BE_CANCELLED', `Cannot cancel order with status: ${order.status}`);
    }
   ...
   */
  async getOrderDetailsAdmin(orderId: string) {
    const order = await prisma.order.findUnique({
      where: { id: orderId },
      include: {
        orderItems: true,
   ...
   */
  async updateOrderStatusAdmin(orderId: string, status: any) {
    const order = await prisma.order.findUnique({
      where: { id: orderId },
      include: {
        orderItems: {
    ...
    const updated = await prisma.$transaction(async (tx) => {
      if (shouldRestoreStock) {
        for (const item of order.orderItems) {
          ...
          if (item.product.isPreorder) continue;
