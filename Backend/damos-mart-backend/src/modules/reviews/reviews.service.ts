...
export class ReviewsService {
   ...
   */
  async submitReview(userId: string, data: CreateReviewInput, photoUrls: string[] = []) {
    ...
    if (!order) {
      throw new AppError(
        400,
        'INVALID_REVIEW',
        'Cannot review. Ensure the order is completed and contains this product.'
      );
    }
    ...
    if (existingReview) {
      throw new AppError(409, 'ALREADY_REVIEWED', 'You have already submitted a review for this product in this order');
    }
