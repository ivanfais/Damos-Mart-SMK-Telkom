...
class _CatalogScreenState extends State<CatalogScreen> {
  ...
  // Semua kategori tampil di katalog termasuk Atribut Sekolah
  ...
  String _chipForCategory(String categoryId, List<CategoryModel> cats) {
    if (categoryId.isEmpty) return 'Semua Produk';
    final cat = cats.where((c) => c.id == categoryId).firstOrNull;
    if (cat == null) return 'Semua Produk';
    final n = cat.name.toLowerCase();
    if (n.contains('makan')) return 'Makanan';
    if (n.contains('minum')) return 'Minuman';
    if (n.contains('tulis')) return 'Alat Tulis';
    if (n.contains('atribut') || n.contains('sekolah')) return 'Atribut Sekolah';
    return 'Semua Produk';
  }

  String _categoryIdForChip(String chip, List<CategoryModel> cats) {
    if (chip == 'Semua Produk') return '';
    final kw = switch (chip) {
      'Makanan'         => 'makan',
      'Minuman'         => 'minum',
      'Alat Tulis'      => 'tulis',
  ...
  Widget _buildCategorySection(ProductState state) {
    ...
    final selectedChip = _chipForCategory(selectedCategoryId, categories);
          ...
          Wrap(
            ...
            children: _chipLabels.map((label) {
              ...
              return GestureDetector(
                onTap: () {
                  final id = _categoryIdForChip(label, categories);
